...
if (mysqli_num_rows($result) == 0) {
  ...
  <div class='card-body'>
  <h5 class='card-title'> {$row["id"]} {$row["title"]} {$row["author_first_name"]} {$row["author_last_name"]}</h5>  {$row["image"]} {$row["isbn"]} {$row["short_description"]} {$row["type"]}  {$row["publisher_name"]} {$row["publisher_address"]} {$row["publish_date"]}{$row["status"]} 
